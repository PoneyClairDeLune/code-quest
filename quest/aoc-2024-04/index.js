...
let matchXmas = (bin2d, posX, posY, direction) => {
	/*
...
for (let y = 0; y < testH; y ++) {
	for (let x = 0; x < testW; x ++) {
		console.debug(`OOB yet at (${x}, ${y}) ${testD}? ${matchXmas(testData, x, y, testD)}`);
	};
};*/
...
for (let occuredX of firstOccurs) {
	for (let direction = 0; direction < 8; direction ++) {
		let matchResult = matchXmas(flatData, ...occuredX, direction);
